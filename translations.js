...
existing.addEventListener("change",()=>changeLanguage(existing.value));
if(!document.getElementById("pd-status")){const status=document.createElement("div");status.id="pd-status";protect(status);document.body.appendChild(status)}
updateUI();
return;
}
if(document.getElementById("pd-language-button"))return;
const b=document.createElement("button");b.id="pd-language-button";b.type="button";protect(b);
const overlay=document.createElement("div");overlay.id="pd-language-overlay";protect(overlay);
const modal=document.createElement("div");modal.id="pd-language-modal";
const head=document.createElement("div");head.className="pd-head";
const h=document.createElement("h2");h.id="pd-title";
const close=document.createElement("button");close.className="pd-close";close.type="button";close.textContent="×";head.append(h,close);
const intro=document.createElement("p");intro.className="pd-intro";intro.id="pd-intro";
const grid=document.createElement("div");grid.className="pd-grid";
LANGUAGES.forEach(l=>{const o=document.createElement("button");o.type="button";o.className="pd-option";o.dataset.language=l.code;o.innerHTML=`<span>${l.label}</span><span class="pd-code">${l.short}</span>`;o.onclick=()=>changeLanguage(l.code);grid.appendChild(o)});
modal.append(head,intro,grid);overlay.appendChild(modal);
const status=document.createElement("div");status.id="pd-status";protect(status);
b.onclick=()=>{overlay.classList.add("open");document.body.style.overflow="hidden"};close.onclick=closeMenu;overlay.onclick=e=>{if(e.target===overlay)closeMenu()};
document.body.append(b,overlay,status);updateUI()
}
...
function updateUI(){const l=lang(language),u=UI[language]||UI.en,b=document.getElementById("pd-language-button"),select=document.getElementById("languageSelect");if(b)b.textContent="🌐 "+l.short;if(select&&select.value!==language)select.value=language;const h=document.getElementById("pd-title"),i=document.getElementById("pd-intro");if(h)h.textContent=u[0];if(i)i.textContent=u[1];document.querySelectorAll(".pd-option").forEach(x=>x.classList.toggle("active",x.dataset.language===language))}
